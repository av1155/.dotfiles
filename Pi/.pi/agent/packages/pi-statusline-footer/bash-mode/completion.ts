...
function getGitSuggestions(ctx: TokenContext, cwd: string): ExtendedCompletionItem[] {
    ...
    if (ctx.tokenIndex <= 1) {
        return GIT_SUBCOMMANDS.filter((command) => command.startsWith(ctx.token)).map(
            (command) => ({
                value: command,
                label: command,
                replacement: command,
...
export class OneOffBashAutocompleteProvider implements AutocompleteProvider {
    ...
    applyCompletion(
        ...
        if (!isExtendedCompletionItem(_item)) {
            throw new Error("Expected an extended completion item for one-off bash autocomplete");
        }

        return applyExtendedCompletion(_lines, _cursorLine, _item);
    }
...
export class OneOffAwareAutocompleteProvider implements AutocompleteProvider {
    private readonly defaultProvider: AutocompleteProvider | undefined;
    private readonly oneOffBashProvider: OneOffBashAutocompleteProvider;

    constructor(
        defaultProvider: AutocompleteProvider | undefined,
        oneOffBashProvider: OneOffBashAutocompleteProvider,
    ) {
    ...
    getSuggestions(
        ...
        if (this.oneOffBashProvider.shouldTriggerFileCompletion(lines, cursorLine, cursorCol)) {
