...
export class TerminalSplitCompositor {
    ...
    jumpToRootLine(targetLine: number): boolean {
        if (this.disposed || this.hasVisibleOverlay() || !Number.isFinite(targetLine)) return false;

        const target = Math.max(0, Math.floor(targetLine));
