...
export function renderFixedEditorCluster(input: FixedEditorClusterInput): FixedEditorClusterRender {
    ...
    const promptFooterLines = normalizeLines(input.promptFooterLines, width);
    ...
    const promptFooter = takeTail(promptFooterLines, remaining);
    remaining -= promptFooter.length;
    ...
    const statusTopSpacer = status.length > 0 && top.length > 0 && remaining > 0 ? [""] : [];
    remaining -= statusTopSpacer.length;
