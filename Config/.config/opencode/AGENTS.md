...
Available globally across `~/.config/opencode/skills/`, `~/.claude/skills/`, and `~/.agents/skills/`. Use `find-skills` to browse and load on demand. Repository-local skills override globals.
...
Use the `playwright` MCP for browser-based smoke checks and screenshots.
