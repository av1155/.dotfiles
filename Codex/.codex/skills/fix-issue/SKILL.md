---
name: fix-issue
description: Investigate a bug or problem thoroughly, plan the root-cause fix, implement the smallest safe change, and verify it. Use when the user says fix this, investigate this, debug this, triage this, address this bug, handle this report, or provides a GitHub issue/PR/discussion/security URL, Linear issue, stack trace, error message, or plain-text bug description.
---
...
```bash
gh issue view <issue-number> --json title,body,labels,assignees,comments,url 2>/dev/null
```
...
- **Bug spans many files, subsystems, or involves external services**: use Codex subagents in parallel when available.
...
- **Subagent 1: Code tracing.** Read every source file involved in the problem. Trace the full code path from entry point to the affected behavior. Map the call chain, data flow, and state mutations. Report findings with specific file:line references. Do not claim anything about a file you have not opened.

- **Subagent 2: Test landscape.** Read existing tests covering the area. Understand what is already tested and what is not. Identify the gap that let this bug through — that gap is usually where the reproducing test in Phase 3b belongs.
...
- Project skills, commands, or scripts named review, check, lint, test
