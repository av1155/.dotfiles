...
project's install command before proceeding. Use the rebase skill for the full
...
workmux send agent-a "Use the merge skill to finish this branch"  # send a skill request
...
Use the merge skill to commit, rebase onto the base branch, and merge in one
...
3. Use the open-pr skill to write a PR description and open in browser
4. After PR is merged remotely, clean up with `workmux rm --gone`

### Delegating tasks

Use the worktree skill to spin off tasks into parallel worktree agents. The
