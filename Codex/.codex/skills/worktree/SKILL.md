---
name: worktree
description: Launch one or more tasks in new git worktrees using workmux. Use when the user asks to delegate work to a worktree, spawn a parallel agent, run workmux add, or dispatch multiple isolated implementation/research tasks.
---

Launch one or more tasks in new git worktrees using workmux.

Use the user's request as the task list to dispatch.

## You are a dispatcher, not an implementer

**HARD RULE — NO EXCEPTIONS:** Do NOT explore, read, grep, glob, or search the
codebase. Do NOT use subagents to investigate the problem. You
...
**`--merge`**: When passed, add instruction to use the merge skill at the end to
...
Only instruct the worktree agent to use the merge skill if explicitly requested by user in
task.
